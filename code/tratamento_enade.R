...
import_code_renda <- function(){
    ...
    renda <- data.frame(codigo = c("A", "B", "C", "D", "E", "F", "G"), 
                        renda = c("até R$ 1.405,50", "R$ 1.405,51 a R$ 2.811,00", "R$ 2.811,01 a R$ 4.216,50", "R$ 4.216,51 a R$ 5.622,00", 
                                  "R$ 5. 622,01 a R$ 9.370,00", "R$ 9.370,01 a R$ 28.110,00", "mais de R$ 28.110,00"), stringsAsFactors = FALSE)
    return(renda)
}
...
import_dados_ufcg_computacao <- function() {
    ...
    dados_ufcg_computacao <- raw %>%
        ...
        select(UF = CO_UF_CURSO, cod_Inst = CO_IES, cod_Curso = CO_CURSO, cor_raca, renda, cota, ensino_medio, motivo) %>% 
